...
const ClassificationTable = ({ data, taxon, style,  pathToTree }) => (
  <div style={style}> {_.reverse([...data]).map(t => (
   ...
   {taxon && <a onClick={() => {window.location.href =  `${pathToTree}?taxonKey=${taxon.id}`}} dangerouslySetInnerHTML={{ __html: taxon.labelHtml }} />}
